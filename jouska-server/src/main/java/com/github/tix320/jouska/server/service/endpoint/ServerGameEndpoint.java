...
public class ServerGameEndpoint {
	...
	@Endpoint("info")
	@NeedAuthentication
	public List<GameView> getGames(@ClientID long clientId) {
		return GameManager.getGames().stream().map(gameInfo -> {
			GameSettings settings = gameInfo.getSettings();
			return new GameView(gameInfo.getId(), settings.getName(), gameInfo.getConnectedPlayers().size(), settings.getPlayersCount(),
					settings.getTurnDurationSeconds(), settings.getGameDurationMinutes());
